<x-layout>
    ...
    {{-- Skills Section --}}
    <section class="border-b border-slate-200/50 dark:border-white/10 bg-slate-50 dark:bg-slate-950 py-20 transition-colors duration-300">
        <div class="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
            ...
            @if($skills->isEmpty())
                <p class="text-center text-lg text-slate-500 dark:text-slate-400 lang-text"
                    ...
                    @foreach($categoryConfig as $catName => $config)
                        @if($grouped->has($catName) && $catName !== 'Additional Skills')
                            <div class="group rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 p-6 text-center transition hover:border-emerald-500/30 hover:bg-slate-100/50 dark:hover:bg-white/10 transition-colors duration-300">
                                ...
                                <h3 class="mb-2 font-semibold text-slate-900 dark:text-white lang-text"
                ...
                @if($grouped->has('Additional Skills'))
                    <div class="mt-16">
                        <div class="mb-8 text-center">
                            <h3 class="mb-4 text-2xl font-bold text-slate-900 dark:text-white lang-text"
                                data-en="Additional Skills" data-np="थप सीपहरू">Additional Skills</h3>
                            <div class="mx-auto h-1 w-16 rounded-full bg-gradient-to-r from-transparent via-emerald-400 to-transparent"></div>
                        </div>
                        <div class="flex flex-wrap justify-center gap-3">
                            @foreach($grouped['Additional Skills'] as $skill)
                                <span class="rounded-full border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 px-4 py-2 text-sm text-slate-600 dark:text-slate-300 transition hover:border-emerald-500/30 hover:bg-slate-50 dark:hover:bg-white/10 transition-colors duration-300 lang-text"
                                    data-en="{{ $skill->name }}"
